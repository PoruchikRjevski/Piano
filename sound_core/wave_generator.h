...
public:
    static void genNoteData(NoteData &data)
    {
        ...
        for (unsigned long i = 1; i <= data._dataSize; i += 2) {
            data._data[i - 1] = genValue(data._freq,
                                         time);
            data._data[i] = genValue(data._freq + FREQ_STEREO_SHIFT,
                                     time);
    ...
    static float genValue(float freq,
                          unsigned long t)
    {
        float phase = _sinMult * t * freq;
        float res = 0.4f * sin(phase) * exp(SUSTAIN_EXP_POW * t);
        res      += 0.6f * sin(FREQ_STEREO_SHIFT * phase) * exp(SUSTAIN_EXP_POW * t);
